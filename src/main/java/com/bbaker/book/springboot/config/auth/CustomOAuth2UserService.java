...
import com.bbaker.book.springboot.config.auth.dto.OAuthAttributes;
...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        /*registrationId
           ...
           - 구글, 네이버등 로그인 연동 시 구글인지 네이버 인지 구분하기 위해 사용.*/
        String registrationId = userRequest.getClientRegistration().getRegistrationId();

        /*userNameAttributeName
           - OAuth2 로그인 진행 시 키가 되는 필드값을 이야기함. Primary Key와 같은 의미.
           - 구글의 경우 기본적으로 코드를 지원하지만, 네이버,카카오 등은 하지 않음. 구글 기본 코드는 "sub"
        ...
        /*SessionUser
           ...
           - 왜 User 클래스를 쓰지 않고 새로 만드는지....
             : 세션에 저장하려면 User 클래스에 직렬화를 구현해야하는데 그렇게 되면 User 클래스가 엔티티이기 때문에
               언제 다른 엔티티와 관계가 형성될지 모름. 그러면 나중에 수정할때 귀찮음. 그래서 직렬화 기능을 가진 세션 dto를 하나 추가.*/
